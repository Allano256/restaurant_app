...
class ReserveForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            "date_of_month": forms.DateInput(
                attrs={"type": "date", "min": date.today().isoformat()}
            ),
            "time_of_day": TimeInput(),
            "number_of_guests": forms.NumberInput(
                attrs={"min": "1", "max": "10"}),
            "phone_user": forms.TextInput(
                attrs={"pattern": "[0-9]+", "title": "Enter numbers only"}
            ),
        }
        ...
        def number_guests(self):
            ...
            if not (1 <= guests_allowed <= 10):
                raise forms.ValidationError(
                    "Number of guests can be between 1 and 10 only!"
                )
...
class EditForm(forms.ModelForm):
    ...
    class Meta:
        ...
        def number_guests(self):
            ...
            if guests_allowed < 0:
                raise forms.ValidationError(
                    "Please enter numbers from 1 upwards!")
                if not (1 <= guests_allowed <= 10):
                    raise forms.ValidationError(
                        "Number of guests can only be between 1 and 10 only!"
                    )
