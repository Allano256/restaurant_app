...
def starting_page(request):
    """
    This function takes the user to the landing
...
class ReserveView(LoginRequiredMixin, CreateView):
    """
    This class will save the data entered by the user
    after submission and redirect to success url!
    """
    ...
    def form_valid(self, form):
        ...
        if booking_date == date.today() and booked_time < current_time:
            form.add_error(
                "time_of_day",
                "The time cannot be in the past for today's bookings."
            )
        ...
        messages.success(
            ...
            "a table has been reserved for you.",
        )
    ...
    def get_success_url(self):
        return reverse_lazy(
             "booking:single_reservation", args=[self.object.pk])
...
def single_reservation(request, pk):
    """
    This  view will output a single result of the reservation made
    ...
    return render(
        request, "booking/single_reservation.html",
    ...
    if request.method == "POST":
        ...
        if form.is_valid() and change.user == request.user:
            ...
            messages.add_message(
                ...
                "Error updating your reservation!"
            )
